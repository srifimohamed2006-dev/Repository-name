...
// ---------- تحميل البيانات القابلة للتعديل من لوحة التحكم ----------
async function loadStoreData() {
  ...
  return { products: PRODUCTS, settings: settingsData };
}

function applySettings(settings) {
  const root = document.documentElement;
  if (settings.colorInk) root.style.setProperty("--ink", settings.colorInk);
  if (settings.colorCream) root.style.setProperty("--cream", settings.colorCream);
  if (settings.colorGold) root.style.setProperty("--gold", settings.colorGold);
  if (settings.colorGold2) root.style.setProperty("--gold-2", settings.colorGold2);
  if (settings.colorMaroon) root.style.setProperty("--maroon", settings.colorMaroon);
  window.SHOP_SETTINGS = settings;
}
...
function renderHeader(mountId) {
  ...
  const shopName = (window.SHOP_SETTINGS && window.SHOP_SETTINGS.shopName) || "M-SHOP";
  el.innerHTML = `
    <div class="header-row container">
      <a class="brand" href="${buildUrl("index.html")}">
        <img src="assets/img/logo.png" alt="شعار ${shopName}" />
...
function renderFooter(mountId) {
  ...
  el.innerHTML = `
    <div class="container">
      <div class="brand-name" style="justify-content:center;display:flex;gap:6px;margin-bottom:8px;">${shopName}</div>
      <div>جميع المنتجات معروضة عبر روابط الشراء الرسمية للمورّد. الأسعار قابلة للتغيير.</div>
      <div style="margin-top:14px;"><a href="admin/" style="opacity:0.45;font-size:0.78rem;">لوحة التحكم</a></div>
    </div>
  `;
}
